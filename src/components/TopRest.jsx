...
const TopRest = () => {
  ...
  const [itemsVisible, setItemVisible] = useState(2); 
  ...
  const nextSlide = () => {
    setItems()
    const maxSlideIndex = Math.ceil(data.length / itemsVisible) - 1;
    ...
    <div className="max-w-[1200px] mx-auto my-6">
      <div className="flex items-center justify-between md:mx-0 mx-3">
        <div className="text-[25px] font-bold">
          Top Restaurants in Bashundhara R/A?
        </div>
        <div className="flex gap-2">
          <div
            className="w-[30px] h-[30px] bg-[#e2e2e7] rounded-full flex justify-center items-center cursor-pointer"
            onClick={prevSlide}
          >
            <FaArrowLeft />
          </div>
      ...
      <div className="flex gap-5 md:mx-0 mx-3"> 
        {data
          .slice(slide * itemsVisible, slide * itemsVisible + itemsVisible)
          .map((d, i) => {
            return <Card {...d} customClass = {'md:w-[273px] w-[180px] shrink-0 grow mt-6'} key={i}  />;
          })}
      </div>
